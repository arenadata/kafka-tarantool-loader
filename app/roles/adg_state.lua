...
local function init_space_delete_batch_storage()
    local delete_space_batch = box.schema.space.create("_DELETE_SPACE_BATCH", {
        format = {
...
local function init_ddl_queue_space()
    ...
    box.begin()
    local space = box.schema.space.create("_DDL_QUEUE", {
        format = {
...
local function init_kafka_callbacks_logs()
    ...
    local cb_function_call_log_seq = box.schema.sequence.create(
        "KAFKA_CALLBACK_FUNCTIONS_SEQ",
        { if_not_exists = true }
    )

    local cb_function_call_log = box.schema.space.create("_KAFKA_CALLBACK_FUNCTIONS_LOG", {
        format = {
...
local function init_kafka_error_msgs()
    ...
    local err_kafka_err = box.schema.space.create("_KAFKA_ERROR_MSG", {
        format = {
...
local function insert_kafka_callback_log(topic_name, partition_name, cb_function_name)
    ...
    local _, err = err_state_storage:pcall(function()
...
local function init_callback_function_repo()
    local callback_func = box.schema.space.create("_KAFKA_CALLBACK_FUNCTIONS", {
        format = {
            { "CALLBACK_FUNCTION_NAME", "string" },
...
local function ddl_queue_processor()
    ...
    while true do
        ...
        for _, ddl_operation in ddl_queue_space:pairs() do
            ...
            if
                ddl_operation["OBJ_TYPE"] == enums.obj_type.TABLE
                and ddl_operation["DDL_TYPE"] == enums.ddl_type.CREATE_TABLE
            then
            ...
            elseif
                ddl_operation["OBJ_TYPE"] == enums.obj_type.TABLE
                and ddl_operation["DDL_TYPE"] == enums.ddl_type.DROP_TABLE
            then
                current_state.spaces[ddl_operation["OBJ_NAME"]] = nil
                tables_to_drop[ddl_operation["OBJ_NAME"]] = ddl_operation["ID"]
            elseif
                ddl_operation["OBJ_TYPE"] == enums.obj_type.PREFIX
                and ddl_operation["DDL_TYPE"] == enums.ddl_type.DROP_DATABASE
            then
...
local function delayed_delete_prefix(prefix, wait_res_timeout)
    ...
    local tuple, err
    box.begin()
    tuple, err = space:insert({ nil, enums.obj_type.PREFIX, prefix, enums.ddl_type.DROP_DATABASE, nil })
...
local function delayed_create(spaces, wait_res_timeout)
    ...
    for k, v in pairs(spaces) do
        tuple, err = space:insert({ nil, enums.obj_type.TABLE, k, enums.ddl_type.CREATE_TABLE, v })
        ddl_callbacks[tuple["ID"]] = {}
        ddl_callbacks[tuple["ID"]]["cond"] = fiber.cond()
        ddl_callbacks[tuple["ID"]]["status"] = "created"
    end
...
local function delayed_delete(spaces, wait_res_timeout)
    ...
    box.begin()
    for _, v in pairs(spaces) do
        tuple, err = space:insert({ nil, enums.obj_type.TABLE, v, enums.ddl_type.DROP_TABLE, nil })
        ddl_callbacks[tuple["ID"]] = {}
        ddl_callbacks[tuple["ID"]]["cond"] = fiber.cond()
        ddl_callbacks[tuple["ID"]]["status"] = "created"
    end
