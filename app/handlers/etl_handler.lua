...
local function transfer_data_to_scd_table(req)
    ...
    local ok, err = _G.transfer_data_to_scd_table_on_cluster(
    ...
    if ok then
        return success_repository.return_http_response("API_ETL_TRANSFER_DATA_TO_HISTORICAL_TABLE_001")
    else
        if type(err) == "string" then
            return error_repository.return_http_response(json.decode(err)["errorCode"], err)
        else
            return error_repository.return_http_response("ROLE_VALIDATE_CONFIG_ERR_001", err)
        end
    end
end
...
local function get_scd_table_checksum(req)
    ...
    local is_ok, res = _G.get_scd_table_checksum_on_cluster(
        body.actualDataTableName,
        body.historicalDataTableName,
        body.sysCn,
        body.columnList,
        norm
    )
