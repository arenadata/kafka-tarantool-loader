...
local function unsubscribe(topics)
    ...
    if consumer ~= nil then
        ...
        return error_repository.get_error_code("ADG_KAFKA_CONSUMER_004")
    end
end
...
local function get_message_from_channel()
    local consumed = {}
    while not message_channel:is_empty() do
...
local function poll()
    if consumer ~= nil then
        ...
        error_repository.get_error_code("ADG_KAFKA_CONSUMER_004")
    end
end
...
local function commit(last_messages)
    ...
    if consumer ~= nil then
        for _, v in ipairs(last_messages) do
            ...
            if err ~= nil then
                return false,
                    error_repository.get_error_code("ADG_KAFKA_CONSUMER_003", { error = err, topic = v:topic() })
            end
        end
        return true, nil
    else
