...
local function prepare(query)
    ...
    if type(space_name) ~= 'string' then
        return nil, errors.new("calculate_bucket_id_error",
                               "only selects from single table are supported")
    end
    ...
    if sharding_key == nil then
        return nil, errors.new("calculate_bucket_id_error",
                               "Sharding key not set for space: %s", space)
    end
...
local function get_replicas(query, params)
    ...
    local bucket_id, err = calculate_bucket_id(
