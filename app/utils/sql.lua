...
local mysql = locale({
    ...
    select_expr = C(V("space") ^ 0 * P("*"))
        + C((V("space") ^ 0 * V("table_name") * P(".") * P("*")))
    ...
    column_item = (
            binary_op(V("expr") * V("space") ^ 0 * CL(K("AS")) * V("space") ^ 0 * C(V("column_alias")))
            + V("expr")
        ),

    expr = V("space") ^ 0 * (V("andor") + V("comp") + V("addsub") + V("muldiv") + V("unary") + V("value")),
    ...
    addsub = binary_op(
        (V("muldiv") + V("unary") + V("value")) * V("addsub_op") * (V("addsub") + V("muldiv") + V("unary") + V("value"))
    ),
    muldiv = binary_op((V("unary") + V("value")) * V("muldiv_op") * (V("muldiv") + V("unary") + V("value"))),
    unary = node(V("unary_op") * (V("value"))),
    ...
    comp_op = V("space") ^ 0 * C(P("=") + P(">=") + P(">") + P("<=") + P("<") + P("<>") + P("!=")),

    value = (V("literal_value") + V("column_expr") + V("variable") + V("param")),

    param = param(V("space") ^ 0 * S("?")),

    column_expr = V("space") ^ 0 * C(
        V("three_part_column_expr") + V("two_part_column_expr") + V("single_part_column_expr")
    ),
    ...
    literal_value = V("space") ^ 0
        * (
            ...
            + P("CURRENT_DATE")
            + P("CURRENT_TIMESTAMP")
        ), -- see http://dev.mysql.com/doc/refman/5.5/en/literals.html
    ...
    real_string_literal = P('"') * C((1 - P('"')) ^ 0) * P('"') + P("'") * C((1 - P("'")) ^ 0) * P("'"),
